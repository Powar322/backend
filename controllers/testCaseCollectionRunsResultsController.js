...
exports.getTestCaseCollectionRunsResults = (req, res) => {
    ...
    let sql = 'SELECT result_of_each_TC.id, result_of_each_TC.test_case_id AS resTestCaseId, test_cases.case_id AS testCaseId, test_cases.description AS testCaseDescription, result_of_each_TC.collection_run_id AS collectionRunId, result_of_each_TC.result AS resultId, results_of_tc.name AS resultName from result_of_each_TC\n' +
...
exports.updateTestCaseCollectionRunsResultsById = (req, res) => {
    ...
    const sql = `UPDATE result_of_each_TC SET  result = ? WHERE id = ?`
    db.run(sql, [resultId, id], function(err){
        if (err){
